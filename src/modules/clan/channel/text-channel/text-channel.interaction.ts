...
import {textChannelOverwrite} from '../helpers/channelOverwrite.js'
import ClanChannelManager from '../managers/clan-channel.manager.js'

class TextChannelInteraction extends PublicHandler {
    protected async run({interaction}: ButtonHandlerOptions) {
        ...
        const textChannel = await InteractionUtils.createChannel(interaction, {
            type: ChannelType.GuildText,
            name: await NamePatternUtils.getPattern(clan),
            parent: category.id,
            permissionOverwrites: textChannelOverwrite(interaction.client.user, interaction.guildId, clan.id)
        })
        await ClanChannelManager.create(interaction.guildId, textChannel.id, clan.id, {
            name: await NamePatternUtils.getPattern(clan)
        })
        const embed = (await clanTitleEmbed(clan, interaction.t('clan:clan')))
            .setDescription(interaction.t('clan:channel:text_channel', {
                channel: textChannel
            }))
...
export default new TextChannelInteraction(ClanChannelButtonCustomIds.TextChannel)